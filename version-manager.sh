...
# Автоматическое увеличение patch версии при коммитах
...
echo "=============================================="
echo -e "${YELLOW}ℹ️ Patch версии увеличиваются автоматически при коммитах${NC}"
echo -e "${YELLOW}ℹ️ Major и Minor версии управляются вручную${NC}"
echo ""
...
if [ "$CURRENT_TAG_COMMIT" != "$HEAD_COMMIT" ] && [ -z "$TAG_ON_HEAD" ]; then
    ...
    echo ""
    echo -e "${BLUE}🎯 Выберите действие:${NC}"
    echo "1) Увеличить Major версию (критические изменения)"
    echo "2) Увеличить Minor версию (новые функции)"
    echo "3) Показать информацию о текущей версии"
    echo "4) Отмена"
    ...
    case $choice in
        1|2)
            # Увеличиваем Major или Minor версию
            if [[ $CURRENT_VERSION =~ v([0-9]+)\.([0-9]+)\.([0-9]+) ]]; then
                MAJOR=${BASH_REMATCH[1]}
                MINOR=${BASH_REMATCH[2]}
                PATCH=${BASH_REMATCH[3]}
                ...
                if [ "$choice" = "1" ]; then
                    NEW_VERSION="v$((MAJOR + 1)).0.0"
                    echo -e "${BLUE}📝 Увеличиваем Major версию: $CURRENT_VERSION → $NEW_VERSION${NC}"
                else
                    NEW_VERSION="v$MAJOR.$((MINOR + 1)).0"
                    echo -e "${BLUE}📝 Увеличиваем Minor версию: $CURRENT_VERSION → $NEW_VERSION${NC}"
                fi
            else
                echo -e "${RED}❌ Не удалось распарсить версию: $CURRENT_VERSION${NC}"
                exit 1
        ...
        3)
            ...
            echo -e "${YELLOW}💡 Следующий коммит автоматически увеличит patch версию${NC}"
            exit 0
            ;;
        4)
            echo -e "${YELLOW}❌ Операция отменена${NC}"
